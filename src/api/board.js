...
async function writeArticle(request, success, fail) {
  await server.post("/board", request.board, {
    headers: {
...
async function listArticle(request, success, fail) {
  await server.get("/board/list", request)
  .then(success)
  .catch(fail);
}

async function getArticle(request, success, fail) {
  await server.get(`/board/${request.boardId}`, {
    headers: {
      Authorization: `Bearer ${request.accessToken}`
    }
  })
...
async function modifyArticle(request, success, fail) {
  await server.put(`/board/${request.board.boardId}`, request.board, {
    headers: {
      Authorization: `Bearer ${request.accessToken}`
    }
  })
...
async function deleteArticle(request, success, fail) {
  await server.delete(`/board/${request.boardId}`, {
    headers: {
      Authorization: `Bearer ${request.accessToken}`
    }
  })
